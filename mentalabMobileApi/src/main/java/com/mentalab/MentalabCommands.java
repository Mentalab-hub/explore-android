...
public final class MentalabCommands {
  ...
  public static ExploreDevice connect(String deviceName)
    ...
    if (!deviceName.startsWith("Explore_")) {
      throw new NoConnectionException(
          "Device names must begin with 'Explore_'. Provided device name: " + deviceName
              + ". Exiting.");
    }
  ...
  public static ExploreDevice connect(BluetoothDevice device)
      throws NoConnectionException, NoBluetoothException, CommandFailedException, IOException, InvalidDataException {
   ...
   * permissions docs</a>.
   * <p>
   * Currently, a lot of functionality missing including: blocking on record, setting a duration for
   * recording, masking channels and overwriting previous files.
   */
