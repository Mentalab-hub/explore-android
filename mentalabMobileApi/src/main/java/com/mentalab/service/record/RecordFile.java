...
public class RecordFile {
  ...
  public void createFile(Uri downloads, Context ctx) throws IOException {
    final Uri file = ctx.getContentResolver().insert(downloads, metaData); // thread safe
    if (file == null) {
      throw new IOException("File already exists, please choose another filename. File path: " + downloads);
    }
    this.uri = file;
  }
