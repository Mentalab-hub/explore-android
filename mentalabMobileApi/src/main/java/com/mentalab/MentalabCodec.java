...
public final class MentalabCodec {
  ...
  private static class ParseRawDataTask implements Callable<Void> {
    ...
    public Void call() {
      while (!Thread.currentThread().isInterrupted()) {
        try {
          ...
          final int length = readToInt(btInputStream, 2); // package size in bytes = timestamp + payload + fletcher
    ...
    private Packet createPacket(int pID, int length, double timeStamp) throws IOException {
      ...
      byte[] noFletcherBuffer = Arrays.copyOfRange(buffer, 0, buffer.length - 4); // ignore 4 byte Fletcher
    ...
    private static byte[] readPayload(InputStream i, int noBytesToRead, int initialBufferLength) throws IOException {
