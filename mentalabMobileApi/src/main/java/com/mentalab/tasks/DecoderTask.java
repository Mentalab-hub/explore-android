    ...
    public Void call() {
      ...
      while (!Thread.currentThread().isInterrupted()) {
        try {
          ...
          Packet packet = parsePayloadData(pId, timeStamp, Arrays
              .copyOfRange(buffer, 0, buffer.length - 4));
          if (packet instanceof QueueablePacket){
            pushDataInQueue(packet);
          }
          if (packet instanceof PublishablePacket){
            PubSubManager
                .getInstance().publish(((PublishablePacket) packet).getTopic().toString(), packet);
          }
